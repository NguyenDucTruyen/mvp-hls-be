...
const schemas: Record<string, object> = {
  ...
  CreateSignedVideoUploadRequest: {
    type: 'object',
    required: ['title', 'originalFilename', 'mimeType', 'sizeBytes'],
    properties: {
      title: { type: 'string', maxLength: 255, example: 'My Video' },
      description: {
        type: 'string',
      ...
      data: {
        ...
        properties: {
          ...
          uploadUrl: {
            type: 'string',
            format: 'uri',
            example: 'https://api.cloudinary.com/v1_1/demo/video/upload',
          },
          publicId: { type: 'string', example: 'mvp-hls/raw/video-id' },
          apiKey: { type: 'string', example: '1234567890' },
          timestamp: { type: 'integer', example: 1781111111 },
          signature: { type: 'string', example: 'cloudinary-signature' },
          resourceType: { type: 'string', enum: ['video'], example: 'video' },
          maxFileSize: {
            type: 'integer',
            maximum: 524288000,
            example: 524288000,
          ...
          uploadParams: {
            ...
            properties: {
              public_id: { type: 'string', example: 'mvp-hls/raw/video-id' },
              timestamp: { type: 'integer', example: 1781111111 },
              api_key: { type: 'string', example: '1234567890' },
              signature: { type: 'string', example: 'cloudinary-signature' },
              overwrite: { type: 'boolean', enum: [false], example: false },
            },
          },
        },
      },
  ...
  CompleteSignedVideoUploadRequest: {
    type: 'object',
    required: ['publicId', 'version', 'signature', 'secureUrl'],
    properties: {
      publicId: { type: 'string', example: 'mvp-hls/raw/video-id' },
      version: { type: 'integer', minimum: 1, example: 1781111111 },
      signature: { type: 'string', example: 'cloudinary-response-signature' },
      secureUrl: {
        type: 'string',
        format: 'uri',
        example:
          'https://res.cloudinary.com/demo/video/upload/v1781111111/mvp-hls/raw/video-id.mp4',
      },
    },
  },
...
const paths: Record<string, object> = {
  ...
  '/api/videos/upload/signature': {
    post: {
      ...
      description:
        'Creates a video record and returns signed Cloudinary parameters so the frontend can upload the raw video directly to Cloudinary.',
      operationId: 'createSignedVideoUpload',
      requestBody: {
        required: true,
        content: {
          'application/json': {
            schema: {
              $ref: '#/components/schemas/CreateSignedVideoUploadRequest',
            },
          },
        },
      },
      responses: {
        '201': {
          description: 'Signed upload parameters created',
          content: {
            'application/json': {
              schema: {
                $ref: '#/components/schemas/SignedVideoUploadResponse',
              },
            },
          },
        },
  ...
  '/api/videos/{id}/upload-complete': {
    post: {
      tags: ['Videos'],
      summary: 'Complete direct video upload',
      description:
        'Verifies the Cloudinary upload response, stores the raw asset metadata, and queues the video for HLS transcoding.',
      operationId: 'completeSignedVideoUpload',
      parameters: [UuidParam('id')],
      requestBody: {
        required: true,
        content: {
          'application/json': {
            schema: {
              $ref: '#/components/schemas/CompleteSignedVideoUploadRequest',
            },
          },
        },
      },
      responses: {
        '202': {
          description: 'Upload completion accepted — processing queued',
          content: {
            'application/json': {
              schema: { $ref: '#/components/schemas/VideoUploadResponse' },
            },
        ...
        '409': {
          description: 'Video is not in a state that can complete upload',
          content: {
            'application/json': {
              schema: { $ref: '#/components/schemas/ErrorResponse' },
            },
